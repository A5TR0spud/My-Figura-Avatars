...
function events.entity_init()
  if not animations.model.record:isPlaying() then
    animations.model.record:setSpeed(0.15)
    animations.model.record:play()
  end
end
...
function events.tick()
  ...
  if note % 20 == 0 then
    local emitPos = models.model.root.Body.Grammo.NoteEmit:partToWorldMatrix():apply()
    particles:newParticle("minecraft:note 16 1 1 1", emitPos, 0, 0.05, 0)
  end
  note = note + 1
end
